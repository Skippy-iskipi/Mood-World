...
export default function ChatComponent({ role, onClose, embedded = false }: ChatComponentProps) {
  ...
  return (
    <div className={embedded
      ? "relative flex flex-col h-full bg-white/95 min-w-0 w-full max-w-full rounded-xl border"
      : "fixed inset-0 z-50 flex flex-col bg-white/95 min-h-screen min-w-0 w-full max-w-full"
    }>
      ...
      <div className={embedded ? "flex-1 flex flex-col overflow-hidden w-full min-w-0" : "flex-1 overflow-y-auto space-y-2 p-2 sm:p-4 pb-32 sm:pb-28 w-full min-w-0"}>
        <div className={embedded ? "flex-1 overflow-y-auto space-y-2 p-2 sm:p-4 w-full min-w-0" : undefined}>
          {messages.map(msg => {
        ...
        {/* Input Bar */}
        {embedded ? (
          <div className="w-full bg-white/95 p-2 flex gap-1 border-t items-center rounded-b-xl">
            {/* Emoji Button */}
            <button
